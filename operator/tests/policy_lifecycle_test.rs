...
use patronus_operator::crd::policy::{
    ActionType, MatchCriteria, Policy, PolicyAction, PolicySpec, Protocol, QoSClass, QoSConfig,
};
...
async fn test_policy_creation() {
    ...
    let fetched = policies
        .get("test-policy-voip")
        .await
    ...
    // Cleanup
    let _ = policies
        .delete("test-policy-voip", &DeleteParams::default())
        .await;
}
...
async fn test_policy_priority() {
    ...
    for priority in &priorities {
        let policy = Policy::new(
            &format!("test-policy-pri-{}", priority),
            PolicySpec {
                priority: *priority,
                match_criteria: MatchCriteria {
    ...
    let mut found_priorities: Vec<i32> = policy_list
        .items
        .iter()
...
async fn test_policy_deletion() {
    ...
    // Delete the policy
    policies
        .delete("test-policy-delete", &DeleteParams::default())
        .await
